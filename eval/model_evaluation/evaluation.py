...
def _internal_evaluate_model(
    ...
    z_train = z[:n_train_samples].cpu().numpy()
    z_pred_train = z_pred[:n_train_samples].cpu().numpy()
    z_val_test = z[n_train_samples:].cpu().numpy()
    z_pred_val_test = z_pred[n_train_samples:].cpu().numpy()
    ...
    n_train_batches = len(z_train) // batch_size
...
def compute_relative_performance(
    ...
    """
    clipped_baseline_off_score = np.clip(baseline_scores[1], 0, 1)
...
def evaluate_model(
    ...
    ceiling_r2_scores, ceiling_accuracies = _evaluate_model(z)
    if verbose > 0:
        print("Evaluating model performance.")
    r2_scores, accuracies = _evaluate_model(z_pred)
